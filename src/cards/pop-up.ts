...
export function handlePopUp(context) {
	...
	const createPopUp = () => {
	    if (!context.host) {
		    ...
		    formatedState = stateEntityId ? hass.formatEntityState(hass.states[stateEntityId]) : formatedState || '';
	        ...
	        setTimeout(() => {
		        ...
		        addStyles(hass, context, popUpStyles, customStyles, state, entityId, context.bgColorChanged, '', popUp);
	        }, 0);
	    }
	}
