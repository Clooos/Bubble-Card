...
| `fields.*.warn_text` | string | The warning message displayed while `warn_if` is truthy. Top-level fields render it as an amber warning alert above the field; fields inside a `group` show it as a ⚠️ helper line instead (the nested group form cannot display alerts). |
| `fields.*.default` | any | For text-based sub-fields, the declared default is shown as the input's placeholder so users can see what applies when the field is left empty. |
