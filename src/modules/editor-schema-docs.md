...
A module can ship an `editor_code` block next to its `code` block. It runs once per module load — not per render, but again whenever a background refresh replaces the module definitions (see the rules below) — with the module's id available as `module_id` and the module's own definition object as `module`. It has the same trust level as the module's `code` block, which already executes arbitrary JavaScript in every card render.
...
The block runs once per load, not per render, so the generated schema is static. Config-reactive behavior stays where it already lives: `visible_if`, `warn_if`, variants, and custom selectors.
...
For a complete working module using `editor_code` together with object-selector groups, `visible_if`, `warn_if`, variants and a `context`-aware custom selector, see [`examples/glow-rules.yaml`](../../examples/glow-rules.yaml).
