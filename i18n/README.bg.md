...
**[`Инсталация`](#инсталация)**  **[`Конфигурация`](#конфигурация)**  **[`Предложения за обекти`](#предложения-за-обекти)**  **[`Pop-up`](#pop-up)**  **[`Хоризонтален стек от бутони`](#хоризонтален-стек-от-бутони)**  **[`Бутон`](#бутон)**  **[`Медиен плейър`](#медиен-плейър)**  **[`Щори`](#щори)**  **[`Избор`](#избор)**  **[`Климат`](#климат)**  **[`Календар`](#календар)**  **[`Разделител`](#разделител)**  **[`Празна колона`](#празна-колона)**  **[`Само подбутони`](#само-подбутони)**  **[`Подбутони`](#подбутони)**  **[`Подредби на картата`](#подредби-на-картата)**  **[`Условия`](#условия)**  **[`Действия`](#действия-при-докосване-двойно-докосване-и-задържане)**  **[`Оформление`](#оформление)**  **[`Шаблони`](#шаблони)**  **[`Модули (Modules)`](#модули-modules)**  **[`Локализация`](#локализация)**  **[`Помощ`](#помощ)**  **[`Принос`](#принос)**  **[`Дарения`](#дарения)**
...
## Предложения за обекти

От Home Assistant 2026.6 насам изборът на обект в избора на карти ви предлага няколко готови карти, а Bubble Card добавя към този списък собствени рецепти. Изберете осветление и получавате карта с плъзгач за яркост, плюс варианти с цветна температура, с цвят и с наситеност, когато осветлението ви ги поддържа. Изберете щора и получавате плъзгача за позицията ѝ, изберете медиен плейър и получавате и вариант със списъка на източниците му, изберете прахосмукачка и получавате бутоните ѝ за старт, пауза и връщане към базата. Всяко предложение е обикновена конфигурация на Bubble Card, показана като преглед на живо, така че можете да вземете най-близката и да продължите да я редактирате както обикновено.

Какво ви се предлага зависи от това какво наистина може вашият обект: осветление без канал за яркост получава превключвател вместо плъзгач, щора, която не може да се накланя, не получава вариант с наклон, а обект за климат получава предварителните си режими само когато има такива. Класическите записи следват под предложенията на Bubble Card, когато са приложими: картата, предназначена за този тип обекти, обикновен бутон и плъзгач.

> [!TIP]
> Модулите могат да добавят собствени предложения към този списък, вижте [модулите](#модули-modules).
...
| `trigger` | object или list | По избор | Вижте [условията](#условия) | Отваряне на този изскачащ прозорец, когато условията са изпълнени |
| `trigger_entity` | string | По избор | Всеки обект | Отваряне на този изскачащ прозорец според състоянието на който и да е обект, опростената форма на `trigger` |
...
| `trigger_close` | boolean | По избор | `true` (по подразбиране) или `false` | Затваряне на изскачащия прозорец, когато условията вече не са изпълнени. По подразбиране е `false`, когато използвате по-старата двойка `trigger_entity` и `trigger_state` |
...
| `slider_fill_orientation` | string | Незадължително | `left`, `right`, `top` или `bottom` | Променя посоката на запълване на плъзгача. Отляво надясно, когато не е зададено, огледално в [езиците, които се пишат отдясно наляво](#локализация) |
| `slider_value_position` | string | Незадължително | `right`, `left`, `center` или `hidden` | Позиция на показваната стойност. Отдясно, когато не е зададено, и отляво в [езиците, които се пишат отдясно наляво](#локализация) |
...
| `show_started_events`| boolean | Незадължително     | `true` (по подразбиране) или `false`                     | Показване или скриване на събитията, които са в момента в ход. Многодневните събития се преценяват ден по ден, така че се скрива само текущият ден, а следващите дни остават видими |
| `scrolling_effect`  | boolean | Незадължително | `true` (по подразбиране) или `false` | Позволява на текста да се превърта, когато съдържанието надхвърля размера на контейнера си |
...
| `visibility` | object или list | Незадължителна | Вижте [условия](#условия) | Показва или скрива подбутона според условия |
...
| `css_class` | string | Незадължителна | Всеки низ | Допълнителен CSS клас върху подбутона, за да го насочите в [оформлението](#оформление) си независимо от името му (напр. `My value` дава `.my-value`) |
...
| `--bubble-sub-button-outline` | `box-shadow` | Контур, добавен към подбутон или плъзгач, само когато този елемент се изчертава в същия цвят като картата зад него, което би го направило невидим (задайте `none`, за да го премахнете) |
...
| `--bubble-sub-slider-outline` | `box-shadow` | Контур само на подбутоните плъзгачи, връща се към `--bubble-sub-button-outline` |
...
Някои опции се управляват от условия, написани точно като тези на [условната карта](https://www.home-assistant.io/dashboards/conditional/) на Home Assistant:

- `visibility` върху [подбутон](#подбутони), за да го показва или скрива
- `trigger` върху [изскачащ прозорец](#pop-up), за да се отвори, когато условията са изпълнени
- `checkConditionsMet(conditions, hass)` в [шаблоните](#шаблони) ви, когато ви трябва отговорът в собствения ви код

Оценява се всеки тип условие на Home Assistant: `state`, `numeric_state`, `screen`, `user`, `time`, `location`, `template`, както и групите `and`, `or` и `not`. Работят и условията от конструктора на условия на Home Assistant, тези с имена по домейна си като `sun.is_up`, `light.is_on`, `zone.in_zone` или `temperature.is_value`, с настройките им `target`, `options`, `behavior` и `for`.
...
> [!NOTE]
> Условията се оценяват във вашия браузър, затова малкото от тях, които се нуждаят от сървъра на Home Assistant, не могат да бъдат точни: изгревът и залезът се четат от обекта `sun.sun`, вместо да се преизчисляват, а продължителността `for` се измерва от последната промяна на състоянието, без историята на recorder.
>
> `view_columns` се приема, но винаги е изпълнено, тъй като Bubble Card никога не подрежда колоните на изгледа ви. Тип условие, което Bubble Card не познава, се съобщава веднъж в конзолата на браузъра ви, вместо да се провали мълчаливо, така че можете да различите печатна грешка от липсваща функция.
...
>
> Класът, базиран на името, се променя, когато преименувате подбутон, и се превежда, когато името е преведено. Задайте `css_class` на подбутона, за да получите собствен клас, който никога не се мести, независимо от името му и от езика.
...
- `getWeatherIcon` може да се използва за връщане на икона за времето на базата на състояние, което връща времето. Например можете да направите това `${subButtonIcon[2].setAttribute("icon", getWeatherIcon(hass.states['sensor.weather_forecast_daily'].attributes.forecast[0]?.condition))}`, за да промените иконата на третия подбутон на иконата за днешното време, `.forecast[1]?.condition` е за утре...
  ...
  ```
- `checkConditionsMet(conditions, hass)` връща `true`, когато списък от [условия](#условия) е изпълнен, например `${checkConditionsMet([{condition: 'sun.is_set'}], hass) ? 'block' : 'none'}`.
...
Но тази функция е много по-мощна от това, тя ви позволява сами да добавяте истински функции в редактора на Bubble Card, използвайки всички стандартни опции на [формулярите на Home Assistant](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)!  
Селекторът за обекти (object) е подобрен, за да показва промените на живо и да поддържа атрибутите правилно.

Един модул може също да отговаря на избора на карти в Home Assistant редом с вградените [предложения за обекти](#предложения-за-обекти): използвайте `suggestions` за картите, които може да опише предварително, и `suggestions_code`, когато трябва да бъдат изчислени от вашата система, например изскачащ прозорец, изграден от всички обекти на зоната, към която принадлежи избраният обект. И двата ключа са документирани [тук](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md#entity-suggestions).
...
- **Пишете предложения за обекти** в редактора на модули, в **По избор: предложения за обекти**, за да се предлага модулът ви в избора на карти на Home Assistant. Както правилата, така и изчислените предложения се проверяват, докато пишете, грешка там пречи на запазването, а прегледът показва предложените карти за всеки избран от вас обект
...
5. (По избор) Създайте персонализиран конфигурационен интерфейс в секцията **Редактор** (като избора на цвят на екранната снимка по-горе, пълната документация е налична [тук](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)).
6. (По избор) Напишете своите **Предложения за обекти**, за да се предлага модулът ви в избора на карти на Home Assistant. Панелът проверява написаното, докато пишете, а прегледът му показва самите предложени карти за обекта по ваш избор.
...
Bubble Card говори вашия език. Редакторът ѝ е преведен на 64-те езика, които Home Assistant поддържа, и навсякъде, където Home Assistant вече има дума за нещо, се използва неговата формулировка, така че четете едни и същи термини в двата интерфейса.

В долната част на редактора, до номера на версията, превключвател **Автоматично** следва езика на вашия Home Assistant. Изключете го и целият редактор се връща на английски, което е удобно, за да следвате урок или да съобщите за проблем. Изборът ви се запомня в браузъра.

Тази документация също е преведена, [на 62 езика](languages.md), всички освен британския английски, който чете оригинала. Тези страници са отворени за всички, така че формулировка, която не съответства на вашия Home Assistant, може да бъде поправена с няколко клика. Английската версия остава референцията за самото съдържание.
