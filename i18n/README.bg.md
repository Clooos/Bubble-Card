...
От Home Assistant 2026.6 насам изборът на обект в избора на карти ви предлага няколко готови карти, а Bubble Card отговаря на този въпрос със собствени рецепти. Изберете осветление и получавате карта с плъзгач за яркост, плюс варианти с цветна температура, с цвят и с наситеност, когато осветлението ви ги поддържа. Изберете щора и получавате плъзгача за позицията ѝ, изберете медиен плейър и получавате и вариант със списъка на източниците му, изберете прахосмукачка и получавате бутоните ѝ за старт, пауза и връщане към базата. Всяко предложение е обикновена конфигурация на Bubble Card, показана като преглед на живо, така че можете да вземете най-близката и да продължите да я редактирате както обикновено.

Какво ви се предлага зависи от това какво наистина може вашият обект: осветление без канал за яркост получава превключвател вместо плъзгач, щора, която не може да се накланя, не получава вариант с наклон, а обект за климат получава предварителните си режими само когато има такива. Класическите записи следват под тях, когато са приложими: специализираната карта на домейна, обикновен бутон и плъзгач.
...
| `trigger_close` | boolean | По избор | `true` или `false` | Затваряне на изскачащия прозорец, когато условията вече не са изпълнени (по подразбиране: `true` с `trigger`, `false` с `trigger_state`) |
...
| `slider_value_position` | string | Незадължително | `right`, `left`, `center` или `hidden` | Позиция на показваната стойност. От крайната страна, когато не е зададено, тоест отляво в [езиците, които се пишат отдясно наляво](#локализация) |
...
| `--bubble-sub-button-outline` | `box-shadow` | Контур, добавен към подбутон или плъзгач само когато той се изчертава в същия цвят като картата зад него, което би го направило невидим (задайте `none`, за да го премахнете) |
...
Тази документация също е преведена, [на 62 езика](languages.md). Тези страници са отворени за всички, така че формулировка, която не съответства на вашия Home Assistant, може да бъде поправена с няколко клика. Английската версия остава референцията за самото съдържание.
