...
1. ดาวน์โหลด `bubble-card.zip` จาก[รีลีสล่าสุด](https://github.com/Clooos/Bubble-Card/releases/latest)
2. แตกไฟล์ลงในโฟลเดอร์ `<config>/www` ของคุณ คุณควรได้ `bubble-card.js` และโฟลเดอร์ `translations` อยู่ข้าง ๆ (โฟลเดอร์นั้นเก็บพจนานุกรมของตัวแก้ไข ถ้าไม่มีมัน ตัวแก้ไขจะยังคงเป็นภาษาอังกฤษ)
...
ตั้งแต่ Home Assistant 2026.6 เป็นต้นมา การเลือกเอนทิตีในตัวเลือกการ์ดจะเสนอการ์ดสำเร็จรูปให้คุณสองสามแบบ และ Bubble Card ก็ตอบคำถามนั้นด้วยสูตรของตัวเอง เลือกไฟแล้วคุณจะได้การ์ดที่มีสไลเดอร์ความสว่าง พร้อมกับแบบอุณหภูมิสี แบบสี และแบบความอิ่มตัวของสี เมื่อไฟของคุณรองรับ เลือกม่าน/ประตูแล้วคุณจะได้สไลเดอร์ตำแหน่งของมัน เลือกเครื่องเล่นสื่อแล้วคุณจะได้แบบที่มีรายการแหล่งสัญญาณด้วย เลือกเครื่องดูดฝุ่นแล้วคุณจะได้ปุ่มเริ่ม หยุดชั่วคราว และกลับแท่นชาร์จ ทุกคำแนะนำเป็นการตั้งค่า Bubble Card ปกติที่แสดงเป็นตัวอย่างแบบสด คุณจึงหยิบอันที่ใกล้เคียงที่สุดมาแก้ไขต่อได้ตามปกติ

สิ่งที่คุณได้รับขึ้นอยู่กับว่าเอนทิตีของคุณทำอะไรได้จริง ไฟที่ไม่มีช่องความสว่างจะได้ปุ่มสลับแทนสไลเดอร์ ม่าน/ประตูที่เอียงไม่ได้จะไม่มีแบบการเอียง และเอนทิตีปรับอากาศจะได้โหมดที่ตั้งไว้ล่วงหน้าก็ต่อเมื่อมันมีเท่านั้น รายการแบบดั้งเดิมจะตามมาด้านล่างเมื่อใช้ได้ ทั้งการ์ดเฉพาะของโดเมนนั้น ปุ่มธรรมดา และสไลเดอร์
...
| `trigger_close` | boolean | Optional | `true` หรือ `false` | ปิดป๊อปอัปเมื่อเงื่อนไขไม่เป็นจริงอีกต่อไป (ค่าเริ่มต้น: `true` กับ `trigger` และ `false` กับ `trigger_state`) |
...
| `slider_value_position` | string | ไม่บังคับ | `right`, `left`, `center` หรือ `hidden` | ตำแหน่งของการแสดงค่า อยู่ทางด้านท้ายเมื่อไม่ได้กำหนด จึงอยู่ทางซ้ายใน[ภาษาที่เขียนจากขวาไปซ้าย](#การแปลภาษา) |
...
| `--bubble-sub-button-outline` | `box-shadow` | เส้นขอบที่เพิ่มให้ปุ่มย่อยหรือแถบเลื่อนเฉพาะเมื่อมันมีสีเดียวกับการ์ดที่อยู่ข้างหลัง ซึ่งจะทำให้มองไม่เห็น (ตั้งเป็น `none` เพื่อเอาออก) |
...
5. (ไม่บังคับ) สร้างส่วนติดต่อกำหนดค่าเองในส่วน **Editor** (เช่นตัวเลือกสีในภาพหน้าจอด้านบน เอกสารฉบับเต็มมีให้ [ที่นี่](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/editor-schema-docs.md))
...
เอกสารนี้ก็ถูกแปลเช่นกัน [ใน 62 ภาษา](languages.md) หน้าเหล่านั้นเปิดให้ทุกคนแก้ไข ถ้อยคำที่ไม่ตรงกับ Home Assistant ของคุณจึงแก้ไขได้ในไม่กี่คลิก เวอร์ชันภาษาอังกฤษยังคงเป็นข้อมูลอ้างอิงสำหรับเนื้อหาเอง
