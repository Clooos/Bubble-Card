...
**[`התקנה`](#התקנה)**  **[`תצורה`](#תצורה)**  **[`חלון קופץ`](#חלון-קופץ)**  **[`ערימת כפתורים אופקית`](#ערימת-כפתורים-אופקית)**  **[`כפתור`](#כפתור)**  **[`נגן מדיה`](#נגן-מדיה)**  **[`תריס`](#תריס)**  **[`בחירה`](#בחירה)**  **[`מיזוג אקלים`](#מיזוג-אקלים)**  **[`לוח שנה`](#לוח-שנה)**  **[`מפריד`](#מפריד)**  **[`עמודה ריקה`](#עמודה-ריקה)**  **[`כפתורי משנה בלבד`](#כפתורי-משנה-בלבד)**  **[`כפתורי משנה`](#כפתורי-משנה)**  **[`פריסות הכרטיס`](#פריסות-הכרטיס)**  **[`פעולות`](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה)**  **[`עיצוב`](#עיצוב)**  **[`תבניות`](#תבניות)**  **[`מודולים`](#מודולים)**  **[`עזרה`](#עזרה)**  **[`תרומה לפרויקט`](#תרומה-לפרויקט)**  **[`תרומה כספית`](#תרומה-כספית)**
...
2. הוסיפו את הקובץ הזה לתיקיית `<config>/www` שלכם
...
| `trigger_entity` | string | אופציונלי | כל ישות | פתחו חלון קופץ זה בהתבסס על מצב של כל ישות |
| `trigger_state` | string | אופציונלי (**חובה** אם `trigger_entity` מוגדר) | כל מצב ישות | מצב הישות לפתיחת החלון הקופץ |
| `trigger_close` | boolean | אופציונלי | `true` או `false` (ברירת מחדל) | סגירת החלון הקופץ כאשר `trigger_state` שונה |
...
| `slider_fill_orientation` | string | אופציונלי | `left` (ברירת מחדל), `right`, `top`, `bottom` | שנה את כיוון המילוי של המחוון |
| `slider_value_position` | string | אופציונלי | `right` (ברירת מחדל), `left`, `center`, `hidden` | מיקום תצוגת הערך |
...
| `show_started_events`| boolean | אופציונלי     | `true` (ברירת מחדל) או `false`                     | הצג או הסתר אירועים המתקיימים כעת                                                 |
| `scrolling_effect`  | boolean | אופציונלי | `true` (ברירת מחדל) או `false` | אפשר לטקסט לגלול כאשר התוכן חורג מגודל המכולה שלו |
...
| `visibility` | object or list | אופציונלי | ראו [תנאים](https://www.home-assistant.io/docs/scripts/conditions/) | הצגה או הסתרה של כפתור המשנה בהתאם לתנאים |
