...
| `trigger_close` | boolean | اختیاری | `true` یا `false` | جب شرائط مزید پوری نہ ہوں تو پاپ اپ بند کریں (ڈیفالٹ: `trigger` کے ساتھ `true`، `trigger_state` کے ساتھ `false`) |
...
| `slider_value_position` | string | اختیاری | `right`، `left`، `center` یا `hidden` | قدر ڈسپلے کی پوزیشن۔ غیر متعین ہونے پر اختتامی جانب، یعنی [دائیں سے بائیں لکھی جانے والی زبانوں](#لوکلائزیشن) میں بائیں طرف |
...
| `--bubble-sub-button-outline` | `box-shadow` | کسی ذیلی بٹن یا سلائیڈر پر صرف اسی وقت شامل ہونے والا آؤٹ لائن جب وہ اپنے پیچھے والے کارڈ جیسا ہی رنگ اختیار کر لے، جس سے وہ نظر نہ آئے (اسے ہٹانے کے لیے `none` مقرر کریں) |
...
یہ دستاویزات بھی ترجمہ ہو چکی ہیں، [62 زبانوں میں](languages.md)۔ یہ صفحات سب کے لیے کھلے ہیں، اس لیے کوئی ایسا انداز جو آپ کے اپنے Home Assistant سے میل نہ کھاتا ہو، چند کلکس میں درست کیا جا سکتا ہے۔ مواد کے لیے انگریزی نسخہ ہی حوالہ رہتا ہے۔
