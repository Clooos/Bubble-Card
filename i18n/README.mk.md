...
**[`Инсталација`](#инсталација)**  **[`Конфигурација`](#конфигурација)**  **[`Предлози за ентитети`](#предлози-за-ентитети)**  **[`Скокачки прозорец`](#скокачки-прозорец)**  **[`Хоризонтален стек од копчиња`](#хоризонтален-стек-од-копчиња)**  **[`Копче`](#копче)**  **[`Медиа плеер`](#медиа-плеер)**  **[`Ролетни`](#ролетни)**  **[`Select`](#select)**  **[`Клима`](#клима)**  **[`Календар`](#календар)**  **[`Разделник`](#разделник)**  **[`Празна колона`](#празна-колона)**  **[`Само подкопчиња`](#само-подкопчиња)**  **[`Подкопчиња`](#подкопчиња)**  **[`Распореди на картичките`](#распореди-на-картичките)**  **[`Услови`](#услови)**  **[`Дејства`](#дејства-на-допир-двоен-допир-и-задржување)**  **[`Стилови`](#стилови)**  **[`Шаблони`](#шаблони)**  **[`Модули`](#модули)**  **[`Локализација`](#локализација)**  **[`Помош`](#помош)**  **[`Придонесување`](#придонесување)**  **[`Донирајте`](#донирајте)**
...
1. Преземете го `bubble-card.zip` од [најновото издание](https://github.com/Clooos/Bubble-Card/releases/latest)
2. Отпакувајте го во вашата папка `<config>/www`, треба да добиете `bubble-card.js` и покрај него папка `translations` (во таа папка се речниците на едиторот, без неа едиторот останува на англиски)
...
## Предлози за ентитети

Од Home Assistant 2026.6, изборот на ентитет во бирачот на картички ви нуди неколку готови картички, а Bubble Card на тој список ги додава своите рецепти. Изберете светло и ќе ви биде понудена картичка со лизгач за осветленост, плус варијанти за температура на бојата, боја и заситеност кога вашето светло ги поддржува. Изберете ролетна и добивате лизгач за нејзината позиција, изберете медиа плеер и добивате и варијанта со неговиот список на извори, изберете правосмукалка и ги добивате нејзините копчиња за старт, пауза и враќање на базата. Секој предлог е обична конфигурација на Bubble Card прикажана како жив преглед, така што можете да го земете најблискиот и да продолжите да го уредувате како и обично.

Тоа што ви се нуди зависи од тоа што вашиот ентитет навистина може: светло без канал за осветленост добива прекинувач наместо лизгач, ролетна што не може да се навалува не добива варијанта за навалување, а клима ентитет ги добива своите преддефинирани режими само кога ги има. Класичните ставки следат под предлозите на Bubble Card кога се применливи: картичката посветена на тој тип ентитет, обично копче и лизгач.

> [!TIP]
> Модулите можат да додадат свои предлози на тој список, видете [модули](#модули).
...
| `trigger` | object или list | Опционално | Видете [услови](#услови) | Го отвора овој скокачки прозорец кога условите се исполнети |
| `trigger_entity` | string | Опционално | Кој било ентитет | Го отвора овој скокачки прозорец врз основа на состојбата на кој било ентитет, едноставната форма на `trigger` |
...
| `trigger_close` | boolean | Опционално | `true` (стандардно) или `false` | Го затвора скокачкиот прозорец кога условите повеќе не се исполнети. Наместо тоа, стандардно е `false` кога го користите постариот пар `trigger_entity` и `trigger_state` |
...
| `slider_fill_orientation` | string | Опционално | `left`, `right`, `top` или `bottom` | Промени ја насоката на пополнување на лизгачот. Кога не е зададено, од лево кон десно, огледално во [јазиците од десно кон лево](#локализација) |
| `slider_value_position` | string | Опционално | `right`, `left`, `center` или `hidden` | Позиција на прикажување на вредноста. Кога не е зададено, десно, а лево во [јазиците од десно кон лево](#локализација) |
...
| `show_started_events`| boolean | Опционално     | `true` (стандардно) или `false`                     | Прикажи или скриј ги настаните кои моментално се во тек. Настаните од повеќе дена се проценуваат ден по ден, така што се крие само денот кој е во тек, а деновите што доаѓаат остануваат видливи |
| `scrolling_effect`  | boolean | Опционално | `true` (стандардно) или `false` | Овозможи текстот да се лизга кога содржината ја надминува големината на нивниот контејнер |
...
| `visibility` | object или list | По избор | Видете [услови](#услови) | Прикажи или скриј го подкопчето врз основа на услови |
...
| `css_class` | string | По избор | Која било низа | Дополнителна CSS класа на подкопчето, за да го таргетирате во вашите [стилови](#стилови) без разлика како се вика (на пр. `My value` дава `.my-value`) |
...
Некои опции се управувани со услови, напишани точно како оние на [условната картичка](https://www.home-assistant.io/dashboards/conditional/) на Home Assistant:

- `visibility` на [подкопче](#подкопчиња), за да го прикажете или скриете
- `trigger` на [скокачки прозорец](#скокачки-прозорец), за да се отвори кога условите се исполнети
- `checkConditionsMet(conditions, hass)` во вашите [шаблони](#шаблони), кога одговорот ви треба во сопствениот код

Се проценува секој тип услов на Home Assistant: `state`, `numeric_state`, `screen`, `user`, `time`, `location`, `template`, како и групите `and`, `or` и `not`. Работат и условите на градителот на услови на Home Assistant, оние именувани по својот домен како `sun.is_up`, `light.is_on`, `zone.in_zone` или `temperature.is_value`, со своите поставки `target`, `options`, `behavior` и `for`.
...
> [!NOTE]
> Условите се проценуваат во вашиот прелистувач, па оние неколкуте на кои им треба серверот на Home Assistant не можат да бидат точни: изгрејсонцето и зајдисонцето се читаат од ентитетот `sun.sun` наместо повторно да се пресметуваат, а траењето `for` се мери од последната промена на состојбата, без историјата на recorder.
>
> `view_columns` се прифаќа, но секогаш поминува, бидејќи Bubble Card никогаш не е тој што ги распоредува колоните на вашиот приказ. Тип услов што Bubble Card не го познава се пријавува еднаш во конзолата на вашиот прелистувач наместо тивко да откаже, така што можете да разликувате печатна грешка од функција што недостасува.
...
>
> Класата базирана на име се менува кога ќе го преименувате подкопчето, а се преведува кога се преведува името. Поставете `css_class` на подкопчето за да добиете сопствена класа која никогаш не се менува, без разлика како се вика и без разлика на јазикот.
...
- `getWeatherIcon` може да се користи за да врати икона за времето врз основа на состојба што ја враќа временската прогноза. На пример, можете да направите вака `${subButtonIcon[2].setAttribute("icon", getWeatherIcon(hass.states['sensor.weather_forecast_daily'].attributes.forecast[0]?.condition))}` за да ја смените иконата на третото подкопче со денешната икона за времето, `.forecast[1]?.condition` е за утре...
  ...
  ```
- `checkConditionsMet(conditions, hass)` враќа `true` кога список од [услови](#услови) е исполнет, на пример `${checkConditionsMet([{condition: 'sun.is_set'}], hass) ? 'block' : 'none'}`.
...
Но оваа функција е многу помоќна од тоа, ви овозможува сами да додавате вистински функции во едиторот на Bubble Card, користејќи ги сите стандардни опции на [Home Assistant формата](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)!  
Селекторот на објекти е подобрен за да прикажува живи промени и правилно да поддржува атрибути.

Еден модул може и да одговори на бирачот на картички на Home Assistant, покрај вградените [предлози за ентитети](#предлози-за-ентитети): користете `suggestions` за картичките што може да ги опише однапред, а `suggestions_code` кога тие мора да се пресметаат од вашата поставеност, на пример скокачки прозорец изграден од сите ентитети на просторијата на која припаѓа избраниот ентитет. Двата клуча се документирани [тука](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md#entity-suggestions).
...
- **Пишувате предлози за ентитети** во едиторот на модули, под **Опционално: предлози за ентитети**, за вашиот модул да биде понуден во бирачот на картички на Home Assistant. И правилата и пресметаните предлози се проверуваат додека пишувате, грешка таму спречува зачувување, а прегледот ги прикажува предложените картички за кој било избран ентитет
...
5. (По желба) Создадете сопствен интерфејс за конфигурација во делот **Editor** (како бирачот на бои на сликата погоре, целосна документација достапна [тука](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)).
6. (По желба) Напишете ги вашите **Предлози за ентитети** за вашиот модул да биде понуден во бирачот на картички на Home Assistant. Панелот проверува што пишувате додека куцате, а неговиот преглед ги прикажува самите предложени картички за ентитетот по ваш избор.
7. Кликнете **Save**.
...
Bubble Card го зборува вашиот јазик. Неговиот едитор е преведен на 64-те јазици што ги поддржува Home Assistant, и секаде каде што Home Assistant веќе има збор за нешто, се презема неговата формулација, така што ги читате истите поими во двата интерфејса.

На дното на едиторот, до бројот на верзијата, прекинувачот **Автоматски** го следи јазикот на вашиот Home Assistant. Исклучете го и целиот едитор се враќа на англиски, што е практично кога следите упатство или пријавувате проблем. Вашиот избор се памети во прелистувачот.

И оваа документација е преведена, [на 62 јазици](languages.md), на сите освен на британски англиски, кој го прикажува оригиналот. Тие страници се отворени за секого, така што формулација што не одговара на вашиот Home Assistant може да се поправи со неколку клика. Англиската верзија останува референца за самата содржина.
