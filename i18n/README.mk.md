...
Од Home Assistant 2026.6, изборот на ентитет во бирачот на картички ви нуди неколку готови картички, а Bubble Card на тоа прашање одговара со свои рецепти. Изберете светло и ќе ви биде понудена картичка со лизгач за осветленост, плус варијанти за температура на бојата, боја и заситеност кога вашето светло ги поддржува. Изберете ролетна и добивате лизгач за нејзината позиција, изберете медиа плеер и добивате и варијанта со неговиот список на извори, изберете правосмукалка и ги добивате нејзините копчиња за старт, пауза и враќање на базата. Секој предлог е обична конфигурација на Bubble Card прикажана како жив преглед, така што можете да го земете најблискиот и да продолжите да го уредувате како и обично.

Тоа што ви се нуди зависи од тоа што вашиот ентитет навистина може: светло без канал за осветленост добива прекинувач наместо лизгач, ролетна што не може да се навалува не добива варијанта за навалување, а клима ентитет ги добива своите преддефинирани режими само кога ги има. Класичните ставки следат под нив кога се применливи: посветената картичка на доменот, обично копче и лизгач.
...
| `trigger_close` | boolean | Опционално | `true` или `false` | Го затвора скокачкиот прозорец кога условите повеќе не се исполнети (стандардно: `true` со `trigger`, `false` со `trigger_state`) |
...
| `slider_value_position` | string | Опционално | `right`, `left`, `center` или `hidden` | Позиција на прикажување на вредноста. Кога не е зададено, на крајната страна, значи лево во [јазиците од десно кон лево](#локализација) |
...
| `--bubble-sub-button-outline` | `box-shadow` | Контура што се додава на подкопче или лизгач само кога тоа се исцртува со истата боја како картичката зад него, што би го направило невидливо (поставете `none` за да ја отстраните) |
...
Еден модул може и да одговори на бирачот на картички на Home Assistant, покрај вградените [предлози за ентитети](#предлози-за-ентитети): користете `suggestions` за картичките што може да ги опише однапред, а `suggestions_code` кога тие мора да се пресметаат од вашата поставеност, на пример скокачки прозорец изграден од сите ентитети на просторијата на која припаѓа избраниот ентитет. Двата клуча се документирани [тука](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/editor-schema-docs.md#entity-suggestions).
...
И оваа документација е преведена, [на 62 јазици](languages.md). Тие страници се отворени за секого, така што формулација што не одговара на вашиот Home Assistant може да се поправи со неколку клика. Англиската верзија останува референца за самата содржина.
