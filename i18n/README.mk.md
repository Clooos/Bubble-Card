...
2. Додајте ја оваа датотека во вашата папка `<config>/www`. За да го добиете едиторот на вашиот јазик, преземете го и `bubble-card-<lang>.json` од [папката dist](https://github.com/Clooos/Bubble-Card/tree/main/dist), на пример `bubble-card-fr.json`, и ставете го покрај `bubble-card.js` (без него едиторот останува на англиски)
