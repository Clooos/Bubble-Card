...
**[`Instalación`](#instalación)**  **[`Configuración`](#configuración)**  **[`Sugerencias de entidad`](#sugerencias-de-entidad)**  **[`Pop-up`](#pop-up)**  **[`Pila de botones horizontal`](#pila-de-botones-horizontal)**  **[`Botón`](#botón)**  **[`Reproductor multimedia`](#reproductor-multimedia)**  **[`Persiana`](#persiana)**  **[`Selector`](#selector)**  **[`Clima`](#clima)**  **[`Calendario`](#calendario)**  **[`Separador`](#separador)**  **[`Columna vacía`](#columna-vacía)**  **[`Solo sub-botones`](#solo-sub-botones)**  **[`Sub-botones`](#sub-botones)**  **[`Diseños de tarjeta`](#diseños-de-tarjeta)**  **[`Condiciones`](#condiciones)**  **[`Acciones`](#acciones-de-toque-doble-toque-y-mantener-presionado)**  **[`Estilos`](#estilos)**  **[`Plantillas`](#plantillas)**  **[`Módulos`](#módulos)**  **[`Localización`](#localización)**  **[`Ayuda`](#ayuda)**  **[`Contribuir`](#contribuir)**  **[`Donar`](#donar)**
...
2. Agrega este archivo a tu carpeta `<config>/www`. Para tener el editor en tu idioma, descarga también `bubble-card-<lang>.json` desde la [carpeta dist](https://github.com/Clooos/Bubble-Card/tree/main/dist), por ejemplo `bubble-card-fr.json`, y ponlo junto a `bubble-card.js` (sin él el editor se queda en inglés)
...
Desde Home Assistant 2026.6, elegir una entidad en el selector de tarjetas te ofrece unas cuantas tarjetas ya listas, y Bubble Card agrega sus propias recetas a esa lista. Elige una luz y se te ofrece una tarjeta con un deslizador de brillo, además de una variante con temperatura de color, otra con color y otra con saturación cuando tu luz las admite. Elige una persiana y obtienes el deslizador de su posición, elige un reproductor multimedia y obtienes también una variante con su lista de fuentes, elige una aspiradora y obtienes sus botones de inicio, pausa y regreso a la base. Cada sugerencia es una configuración normal de Bubble Card mostrada como una vista previa en vivo, así que puedes tomar la más cercana y seguir editándola como siempre.

Lo que se te ofrece depende de lo que tu entidad realmente puede hacer: una luz sin canal de brillo recibe un interruptor en lugar de un deslizador, una persiana que no puede inclinarse no recibe variante de inclinación, y una entidad de clima recibe sus modos preestablecidos solo cuando tiene alguno. Las entradas clásicas siguen debajo de las sugerencias de Bubble Card cuando corresponden: la tarjeta dedicada a ese tipo de entidad, un botón simple y un deslizador.

> [!TIP]
> Los módulos pueden agregar sus propias sugerencias a esa lista, ver [módulos](#módulos).
...
| `trigger_close` | boolean | Opcional | `true` (predeterminado) o `false` | Cierra el pop-up cuando las condiciones dejan de cumplirse. En cambio, el valor predeterminado es `false` cuando usas el par más antiguo `trigger_entity` y `trigger_state` |
...
| `slider_fill_orientation` | string | Opcional | `left`, `right`, `top` o `bottom` | Cambia la dirección de relleno del deslizador. De izquierda a derecha cuando no se define, reflejada en los [idiomas que se escriben de derecha a izquierda](#localización) |
| `slider_value_position` | string | Opcional | `right`, `left`, `center` o `hidden` | Posición de la visualización del valor. A la derecha cuando no se define, y a la izquierda en los [idiomas que se escriben de derecha a izquierda](#localización) |
...
| `show_started_events`| boolean | Opcional     | `true` (predeterminado) o `false`                     | Muestra u oculta los eventos que están actualmente en curso. Los eventos de varios días se evalúan día por día, así que solo se oculta el día en curso y los días siguientes permanecen visibles |
| `scrolling_effect`  | boolean | Opcional | `true` (predeterminado) o `false` | Permite que el texto se desplace cuando el contenido excede el tamaño de su contenedor |
...
>
> `view_columns` se acepta pero siempre se cumple, ya que Bubble Card nunca es quien organiza las columnas de tu vista. Un tipo de condición que Bubble Card no conoce se anuncia una vez en la consola de tu navegador en lugar de fallar en silencio, así distingues un error de escritura de una función que falta.
...
>
> Una clase basada en el nombre cambia cuando renombras un sub-botón, y se traduce cuando el nombre se traduce. Define `css_class` en el sub-botón para tener una clase propia que nunca se mueve, sea cual sea su nombre y sea cual sea el idioma.
...
Pero esta función es mucho más poderosa que eso, te permite agregar funciones reales tú mismo en el editor de Bubble Card, ¡usando todas las opciones predeterminadas del [formulario de Home Assistant](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)!  
...
- **Escribir sugerencias de entidad** en el editor de módulos, en **Opcional: Sugerencias de entidad**, para que tu módulo se ofrezca en el selector de tarjetas de Home Assistant. Tanto las reglas como las sugerencias calculadas se revisan mientras escribes, un error ahí impide guardar, y la vista previa muestra las tarjetas sugeridas para cualquier entidad que elijas
...
5. (Opcional) Crea una interfaz de configuración personalizada en la sección **Editor** (como el selector de color en la captura de pantalla de arriba, documentación completa disponible [aquí](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)).
6. (Opcional) Escribe tus **Sugerencias de entidad** para que tu módulo se ofrezca en el selector de tarjetas de Home Assistant. El panel revisa lo que escribes mientras tecleas, y su vista previa muestra las tarjetas sugeridas mismas para la entidad que elijas.
...
En la parte inferior del editor, junto al número de versión, un interruptor **Automático** sigue el idioma de tu Home Assistant. Desactívalo y todo el editor vuelve al inglés, algo práctico para seguir un tutorial o para reportar un problema. Tu elección se recuerda en tu navegador.

Esta documentación también está traducida, [a 62 idiomas](languages.md), a todos menos al inglés británico, que muestra el original. Esas páginas están abiertas a todos, así que una expresión que no coincide con tu propio Home Assistant se puede corregir en un par de clics. La versión en inglés sigue siendo la referencia para el contenido mismo.
