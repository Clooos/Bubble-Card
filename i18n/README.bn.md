...
1. [সর্বশেষ রিলিজ](https://github.com/Clooos/Bubble-Card/releases/latest) থেকে `bubble-card.zip` ডাউনলোড করুন
2. এটি আপনার `<config>/www` ফোল্ডারে এক্সট্র্যাক্ট করুন, পাশে `bubble-card.js` এবং একটি `translations` ফোল্ডার পাওয়ার কথা (ওই ফোল্ডারে এডিটরের অভিধানগুলো থাকে, সেটি ছাড়া এডিটর ইংরেজিতেই থেকে যায়)
...
Home Assistant 2026.6 থেকে কার্ড পিকারে কোনো এনটিটি বাছাই করলে কয়েকটি তৈরি করা কার্ড দেখানো হয়, আর Bubble Card নিজের রেসিপি দিয়ে সেই প্রশ্নের উত্তর দেয়। একটি লাইট বাছুন, পাবেন উজ্জ্বলতার স্লাইডারসহ একটি কার্ড, আর আপনার লাইট সমর্থন করলে কালার টেম্পারেচার, রঙ ও স্যাচুরেশনের আলাদা সংস্করণও। কভার বাছলে তার পজিশন স্লাইডার, মিডিয়া প্লেয়ার বাছলে তার সোর্স তালিকাসহ একটি সংস্করণও, আর ভ্যাকুয়াম বাছলে তার স্টার্ট, পজ ও ডক বোতাম। প্রতিটি সাজেশন একটি সাধারণ Bubble Card কনফিগারেশন, লাইভ প্রিভিউ হিসেবে দেখানো হয়, তাই সবচেয়ে কাছেরটি নিয়ে আগের মতোই সম্পাদনা চালিয়ে যেতে পারেন।

আপনাকে কী দেখানো হবে তা নির্ভর করে আপনার এনটিটি আসলে কী করতে পারে তার উপর: উজ্জ্বলতার চ্যানেল নেই এমন লাইট স্লাইডারের বদলে টগল পায়, টিল্ট করতে না পারা কভার কোনো টিল্ট সংস্করণ পায় না, আর ক্লাইমেট এনটিটি তার প্রিসেট মোড তখনই পায় যখন সেগুলো থাকে। প্রযোজ্য হলে চিরাচরিত এন্ট্রিগুলো এর নিচে আসে: ডোমেইনের নিজস্ব কার্ড, একটি সাধারণ বাটন এবং একটি স্লাইডার।
...
| `trigger_close` | boolean | ঐচ্ছিক | `true` বা `false` | শর্ত আর পূরণ না হলে পপ-আপ বন্ধ করুন (ডিফল্ট: `trigger`-এর সঙ্গে `true`, `trigger_state`-এর সঙ্গে `false`) |
...
| `slider_value_position` | string | ঐচ্ছিক | `right`, `left`, `center` বা `hidden` | মান প্রদর্শনের অবস্থান। নির্ধারিত না থাকলে শেষ প্রান্তে, অর্থাৎ [ডান থেকে বাঁয়ে লেখা ভাষায়](#লোকালাইজেশন) বাঁ দিকে |
...
| `--bubble-sub-button-outline` | `box-shadow` | সাব-বাটন বা স্লাইডারে যোগ হওয়া রূপরেখা, কেবল তখনই যখন সেটি পেছনের কার্ডের মতো একই রঙে আঁকা হয় এবং অদৃশ্য হয়ে যেত (সরাতে `none` দিন) |
...
5. (ঐচ্ছিক) **এডিটর** সেকশনে একটি কাস্টম কনফিগারেশন UI তৈরি করুন (যেমন উপরের স্ক্রিনশটের কালার পিকার, সম্পূর্ণ ডকুমেন্টেশন পাবেন [এখানে](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/editor-schema-docs.md))।
...
এই ডকুমেন্টেশনও অনূদিত, [62টি ভাষায়](languages.md)। ওই পাতাগুলো সবার জন্য খোলা, তাই আপনার Home Assistant-এর সঙ্গে না মেলা কোনো শব্দচয়ন কয়েক ক্লিকেই ঠিক করা যায়। বিষয়বস্তুর জন্য ইংরেজি সংস্করণই মূল রেফারেন্স থাকে।
