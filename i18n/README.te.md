...
1. [తాజా విడుదల](https://github.com/Clooos/Bubble-Card/releases/latest) నుండి `bubble-card.zip` ను డౌన్‌లోడ్ చేయండి
2. దానిని మీ `<config>/www` ఫోల్డర్‌లో ఎక్స్‌ట్రాక్ట్ చేయండి, దాని పక్కన `bubble-card.js` మరియు `translations` ఫోల్డర్ రావాలి (ఆ ఫోల్డర్‌లో ఎడిటర్ నిఘంటువులు ఉంటాయి, అది లేకుండా ఎడిటర్ ఆంగ్లంలోనే ఉంటుంది)
...
| `trigger_close` | boolean | ఐచ్ఛికం | `true` లేదా `false` | షరతులు ఇక నెరవేరనప్పుడు పాప్-అప్‌ను మూసివేయండి (డిఫాల్ట్: `trigger` తో `true`, `trigger_state` తో `false`) |
...
| `slider_value_position` | string | ఐచ్ఛికం | `right`, `left`, `center` లేదా `hidden` | విలువ ప్రదర్శన యొక్క స్థానం. నిర్వచించనప్పుడు చివరి వైపున, కాబట్టి [కుడి నుండి ఎడమకు రాసే భాషల్లో](#స్థానికీకరణ) ఎడమ వైపున |
...
ఈ డాక్యుమెంటేషన్ కూడా అనువదించబడింది, [62 భాషల్లోకి](languages.md). ఆ పేజీలు అందరికీ తెరిచి ఉన్నాయి, కాబట్టి మీ సొంత Home Assistant తో సరిపోని పదజాలాన్ని కొన్ని క్లిక్‌లలో సరిచేయవచ్చు. విషయానికి సంబంధించి ఆంగ్ల వెర్షనే ప్రామాణికంగా ఉంటుంది.
