...
[<img src="../img/translate.svg" width="17" height="17" align="absmiddle" alt="">](languages.md) **[اقرأ هذه الصفحة بلغة أخرى](languages.md)**
...
**[`التثبيت`](#التثبيت)**  **[`الإعداد`](#الإعداد)**  **[`اقتراحات الكيانات`](#اقتراحات-الكيانات)**  **[`Pop-up`](#pop-up)**  **[`تكديس الأزرار الأفقي`](#تكديس-الأزرار-الأفقي)**  **[`الزر`](#الزر)**  **[`مشغل الوسائط`](#مشغل-الوسائط)**  **[`الستارة`](#الستارة)**  **[`الاختيار`](#الاختيار)**  **[`التكييف`](#التكييف)**  **[`التقويم`](#التقويم)**  **[`الفاصل`](#الفاصل)**  **[`العمود الفارغ`](#العمود-الفارغ)**  **[`الأزرار الفرعية فقط`](#الأزرار-الفرعية-فقط)**  **[`الأزرار الفرعية`](#الأزرار-الفرعية)**  **[`تخطيطات البطاقة`](#تخطيطات-البطاقة)**  **[`الشروط`](#الشروط)**  **[`الإجراءات`](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول)**  **[`التنسيق`](#التنسيق)**  **[`القوالب`](#القوالب)**  **[`Modules`](#modules)**  **[`الترجمة`](#الترجمة)**  **[`المساعدة`](#المساعدة)**  **[`المساهمة`](#المساهمة)**  **[`التبرع`](#التبرع)**
...
منذ الإصدار Home Assistant 2026.6، يعرض عليك منتقي البطاقات بضع بطاقات جاهزة عند اختيار كيان، وتضيف Bubble Card وصفاتها الخاصة إلى تلك القائمة. اختر مصباحًا فتُعرض عليك بطاقة بشريط تمرير للسطوع، إضافة إلى نسخة بدرجة حرارة اللون ونسخة باللون ونسخة بالتشبّع عندما يدعمها مصباحك. اختر ستارة فتحصل على شريط تمرير موضعها، واختر مشغل وسائط فتحصل أيضًا على نسخة بقائمة مصادره، واختر مكنسة فتحصل على أزرار التشغيل والإيقاف المؤقت والعودة إلى القاعدة. كل اقتراح هو إعداد Bubble Card عادي يُعرض كمعاينة حية، فيمكنك أخذ الأقرب إلى حاجتك ومتابعة تعديله كالمعتاد.

ما يُعرض عليك يعتمد على ما يستطيع كيانك فعله بالفعل: مصباح بلا قناة سطوع يحصل على مفتاح تبديل بدل شريط التمرير، وستارة لا تستطيع الإمالة لا تحصل على نسخة إمالة، وكيان تكييف لا يحصل على أوضاعه المعدّة مسبقًا إلا إن كانت لديه. وتأتي المدخلات التقليدية تحت اقتراحات Bubble Card عندما تنطبق: البطاقة المخصصة لهذا النوع من الكيانات، وزر بسيط، وشريط تمرير.

> [!TIP]
> يمكن للوحدات إضافة اقتراحاتها الخاصة إلى تلك القائمة، راجع [Modules](#modules).
...
| `trigger` | object أو list | اختياري | راجع [الشروط](#الشروط) | فتح هذه النافذة المنبثقة عند تحقق الشروط |
| `trigger_entity` | string | اختياري | أي كيان | فتح هذه النافذة المنبثقة بناءً على حالة أي كيان، وهي الصيغة المبسطة من `trigger` |
| `trigger_state` | string | اختياري (**مطلوب** إذا كان `trigger_entity` محددًا) | أي حالة كيان | حالة الكيان التي تفتح النافذة المنبثقة |
| `trigger_close` | boolean | اختياري | `true` (افتراضي) أو `false` | إغلاق النافذة المنبثقة عندما لا تعود الشروط متحققة. تصبح القيمة الافتراضية `false` بدلًا من ذلك عند استخدام الثنائي الأقدم `trigger_entity` و`trigger_state` |
...
| `slider_fill_orientation` | string | اختياري | `left` أو `right` أو `top` أو `bottom` | تغيير اتجاه التعبئة لشريط التمرير. من اليسار إلى اليمين عند عدم التحديد، ومعكوس في [اللغات التي تُكتب من اليمين إلى اليسار](#الترجمة) |
| `slider_value_position` | string | اختياري | `right` أو `left` أو `center` أو `hidden` | موضع عرض القيمة. على اليمين عند عدم التحديد، وعلى اليسار في [اللغات التي تُكتب من اليمين إلى اليسار](#الترجمة) |
...
| `show_started_events`| boolean | اختياري     | `true` (افتراضيًا) أو `false`                     | عرض أو إخفاء الأحداث الجارية حاليًا. تُقيَّم الأحداث الممتدة على عدة أيام يومًا بيوم، فيُخفى اليوم الجاري وحده وتبقى الأيام القادمة ظاهرة |
| `scrolling_effect`  | boolean | اختياري | `true` (افتراضيًا) أو `false` | السماح بتمرير النص عندما يتجاوز المحتوى حجم حاويته |
...
| `visibility` | object أو list | اختياري | راجع [الشروط](#الشروط) | عرض أو إخفاء الزر الفرعي بناءً على الشروط |
...
| `css_class` | string | اختياري | أي نص | فئة CSS إضافية على الزر الفرعي، لاستهدافه في [التنسيق](#التنسيق) لديك مهما كان اسمه (مثلًا `My value` تعطي `.my-value`) |
...
| `--bubble-sub-button-outline` | `box-shadow` | حدّ يُضاف إلى زر فرعي أو شريط تمرير، فقط عندما يُرسم هذا العنصر بنفس لون البطاقة خلفه، ما يجعله غير مرئي (اضبطه على `none` لإزالته) |
...
| `--bubble-sub-slider-outline` | `box-shadow` | حدّ الأزرار الفرعية من نوع شريط التمرير وحدها، ويعود إلى `--bubble-sub-button-outline` |
...
تعتمد بعض الخيارات على شروط تُكتب تمامًا مثل شروط [البطاقة الشرطية](https://www.home-assistant.io/dashboards/conditional/) في Home Assistant:

- `visibility` على [زر فرعي](#الأزرار-الفرعية)، لعرضه أو إخفائه
- `trigger` على [نافذة منبثقة](#pop-up)، لفتحها عند تحقق الشروط
- `checkConditionsMet(conditions, hass)` داخل [القوالب](#القوالب) لديك، عندما تحتاج إلى الإجابة في كودك الخاص

تُقيَّم جميع أنواع شروط Home Assistant: `state` و`numeric_state` و`screen` و`user` و`time` و`location` و`template`، إضافة إلى مجموعات `and` و`or` و`not`. كما تعمل شروط منشئ الشروط في Home Assistant، تلك المسمّاة على اسم نطاقها مثل `sun.is_up` و`light.is_on` و`zone.in_zone` أو `temperature.is_value`، بإعداداتها `target` و`options` و`behavior` و`for`.
...
>
> تتغير الفئة المبنية على الاسم عند إعادة تسمية الزر الفرعي، وتُترجَم عندما يُترجَم الاسم. اضبط `css_class` على الزر الفرعي للحصول على فئة خاصة بك لا تتغير أبدًا، مهما كان اسمه ومهما كانت اللغة.
...
لكن هذه الميزة أقوى من ذلك بكثير، فهي تتيح لك إضافة ميزات فعلية بنفسك في محرر Bubble Card، باستخدام جميع خيارات [نماذج Home Assistant](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md) الافتراضية!  
كما تم تحسين محدد الكائنات (object selector) ليعرض التغييرات مباشرة وليدعم السمات بشكل صحيح.

يمكن للوحدة أيضًا أن تجيب منتقي البطاقات في Home Assistant إلى جانب [اقتراحات الكيانات](#اقتراحات-الكيانات) المدمجة: استخدم `suggestions` للبطاقات التي يمكن وصفها مسبقًا، و`suggestions_code` عندما يجب حسابها من إعدادك، مثل نافذة منبثقة مبنية من كل كيانات المنطقة التي ينتمي إليها الكيان المختار. المفتاحان موثّقان [هنا](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md#entity-suggestions).
...
- **كتابة اقتراحات الكيانات** في محرر الوحدات، ضمن **اختياري: اقتراحات الكيانات**، لتُعرض وحدتك في منتقي البطاقات في Home Assistant. تُفحص القواعد والاقتراحات المحسوبة أثناء الكتابة، ووجود خطأ يمنع الحفظ، وتعرض المعاينة البطاقات المقترحة لأي كيان تختاره
...
5. (اختياري) أنشئ واجهة إعداد مخصصة في قسم **المحرر** (مثل منتقي الألوان في لقطة الشاشة أعلاه، والوثائق الكاملة متاحة [هنا](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/module-documentation.md)).
6. (اختياري) اكتب **اقتراحات الكيانات** الخاصة بك لتُعرض وحدتك في منتقي البطاقات في Home Assistant. تفحص اللوحة ما تكتبه أثناء الكتابة، وتعرض معاينتها البطاقات المقترحة نفسها للكيان الذي تختاره.
7. انقر على **حفظ**.
...
تتحدث Bubble Card لغتك. محررها مترجم إلى 64 لغة يدعمها Home Assistant، وحيثما كان لدى Home Assistant كلمة جاهزة لشيء ما، تُستخدم صياغته نفسها، فتقرأ المصطلحات ذاتها في الواجهتين.

في أسفل المحرر، بجوار رقم الإصدار، يتبع مفتاح **تلقائي** لغة Home Assistant لديك. أوقفه فيعود المحرر كله إلى الإنجليزية، وهو أمر مفيد لمتابعة شرح أو للإبلاغ عن مشكلة. ويُحفظ اختيارك في متصفحك.

هذه الوثائق مترجمة أيضًا، [إلى 62 لغة](languages.md)، وهي كل اللغات ما عدا الإنجليزية البريطانية التي تقرأ النص الأصلي. هذه الصفحات مفتوحة للجميع، لذا يمكن تصحيح أي صياغة لا تطابق Home Assistant لديك ببضع نقرات. وتبقى النسخة الإنجليزية هي المرجع للمحتوى نفسه.
