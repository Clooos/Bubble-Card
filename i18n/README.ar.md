...
منذ الإصدار Home Assistant 2026.6، يعرض عليك منتقي البطاقات بضع بطاقات جاهزة عند اختيار كيان، وتجيب Bubble Card عن هذا السؤال بوصفاتها الخاصة. اختر مصباحًا فتُعرض عليك بطاقة بشريط تمرير للسطوع، إضافة إلى نسخة بدرجة حرارة اللون ونسخة باللون ونسخة بالتشبّع عندما يدعمها مصباحك. اختر ستارة فتحصل على شريط تمرير موضعها، واختر مشغل وسائط فتحصل أيضًا على نسخة بقائمة مصادره، واختر مكنسة فتحصل على أزرار التشغيل والإيقاف المؤقت والعودة إلى القاعدة. كل اقتراح هو إعداد Bubble Card عادي يُعرض كمعاينة حية، فيمكنك أخذ الأقرب إلى حاجتك ومتابعة تعديله كالمعتاد.

ما يُعرض عليك يعتمد على ما يستطيع كيانك فعله بالفعل: مصباح بلا قناة سطوع يحصل على مفتاح تبديل بدل شريط التمرير، وستارة لا تستطيع الإمالة لا تحصل على نسخة إمالة، وكيان تكييف لا يحصل على أوضاعه المعدّة مسبقًا إلا إن كانت لديه. وتأتي المدخلات التقليدية تحتها عندما تنطبق: بطاقة النطاق المخصصة، وزر بسيط، وشريط تمرير.
...
| `trigger_close` | boolean | اختياري | `true` أو `false` | إغلاق النافذة المنبثقة عندما لا تعود الشروط متحققة (الافتراضي: `true` مع `trigger`، و`false` مع `trigger_state`) |
...
| `slider_value_position` | string | اختياري | `right` أو `left` أو `center` أو `hidden` | موضع عرض القيمة. في الجهة النهائية عند عدم التحديد، أي على اليسار في [اللغات التي تُكتب من اليمين إلى اليسار](#الترجمة) |
...
| `--bubble-sub-button-outline` | `box-shadow` | حدّ يُضاف إلى زر فرعي أو شريط تمرير فقط عندما يُرسم بنفس لون البطاقة خلفه، ما يجعله غير مرئي (اضبطه على `none` لإزالته) |
...
هذه الوثائق مترجمة أيضًا، [إلى 62 لغة](languages.md). هذه الصفحات مفتوحة للجميع، لذا يمكن تصحيح أي صياغة لا تطابق Home Assistant لديك ببضع نقرات. وتبقى النسخة الإنجليزية هي المرجع للمحتوى نفسه.
