...
1. 下载这个文件: [bubble-card.js](https://raw.githubusercontent.com/Clooos/Bubble-Card/main/dist/bubble-card.js)
2. 把这个文件放到你的 `<config>/www` 文件夹。要让编辑器显示为你的语言，请同时从 [dist 文件夹](https://github.com/Clooos/Bubble-Card/tree/main/dist)下载 `bubble-card-<lang>.json`，例如 `bubble-card-fr.json`，并把它放在 `bubble-card.js` 旁边 (没有它编辑器就会一直保持英文)
...
从 Home Assistant 2026.6 开始，在卡片选择器中选择一个实体会为你提供几张现成的卡片，而 Bubble Card 会把自己的配方也加进这个列表。选择一个灯光，你会得到一张带亮度滑块的卡片，如果你的灯光支持，还会有色温、颜色和饱和度的变体。选择一个卷帘，你会得到它的位置滑块；选择一个媒体播放器，你还会得到一个带信号源列表的变体；选择一台吸尘器，你会得到它的启动、暂停和返回充电座按钮。每条建议都是一份普通的 Bubble Card 配置，以实时预览的形式呈现，因此你可以选取最接近的一个，然后照常继续编辑。

你会得到什么取决于你的实体实际能做什么：没有亮度通道的灯光得到的是开关而不是滑块，无法倾斜的卷帘不会有倾斜变体，空调实体只有在拥有预设模式时才会得到它们。适用时，经典条目会跟在 Bubble Card 建议的下方：该实体类型的专用卡片、一个普通按钮和一个滑块。
...
| `trigger_close` | boolean | 可选 | `true` (默认) 或 `false` | 当条件不再满足时关闭弹窗。如果你使用较早的 `trigger_entity` 和 `trigger_state` 组合，默认值改为 `false` |
...
| `slider_value_position` | string | 可选 | `right`、`left`、`center` 或 `hidden` | 数值显示的位置。未定义时位于右侧，而在[从右到左的语言](#本地化)中位于左侧 |
...
这份文档同样被翻译了，[共 62 种语言](languages.md)，除英式英语外的每一种语言，英式英语读的是原文。这些页面对所有人开放，因此与你自己的 Home Assistant 不一致的措辞只需点几下就能修正。英文版仍然是内容本身的参考。
