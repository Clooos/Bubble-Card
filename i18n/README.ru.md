...
| `trigger_close` | boolean | Опционально | `true` или `false` | Закрывает всплывающее окно, когда условия перестают выполняться (по умолчанию: `true` с `trigger`, `false` с `trigger_state`) |
...
| `slider_value_position` | string | Опционально | `right`, `left`, `center` или `hidden` | Положение отображения значения. Без указания на конечной стороне, то есть слева в [языках с письмом справа налево](#локализация) |
...
| `--bubble-sub-button-outline` | `box-shadow` | Контур, добавляемый дополнительной кнопке или ползунку только тогда, когда он рисуется тем же цветом, что и карточка позади, из-за чего стал бы невидимым (задайте `none`, чтобы убрать его) |
...
Эта документация тоже переведена, [на 62 языка](languages.md). Эти страницы открыты для всех, поэтому формулировку, которая не совпадает с вашим Home Assistant, можно исправить в пару кликов. Английская версия остаётся ориентиром для самого содержания.
