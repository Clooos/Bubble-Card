...
Модуль также может отвечать подборщику карточек Home Assistant рядом со встроенными [предложениями для сущностей](#предложения-для-сущностей): используйте `suggestions` для карточек, которые он может описать заранее, и `suggestions_code`, когда их нужно вычислить из вашей конфигурации, например всплывающее окно, собранное из всех сущностей той зоны, которой принадлежит выбранная сущность. Оба ключа описаны [здесь](https://github.com/Clooos/Bubble-Card/blob/main/src/modules/editor-schema-docs.md#entity-suggestions).
